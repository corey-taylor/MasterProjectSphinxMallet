...
public class TopicModeller2 {

	WordSequence words;
	InstanceList instances;
	ParallelTopicModel model;
	static String resultText;
	static String[] audioFiles = new String[]{"BC1_1.wav", "BC2.wav", "BC3.wav", "HEALTH1.wav", "HEALTH2.wav"}; 
	...
	static HashMap<String, Float> wordsrank = new HashMap<String, Float>();
	...
	static String[][] INPUTS = {BC1, BC2, BC3, HEALTH1, HEALTH2};		
	public static void main(String[] args) throws Exception {

		TopicModeller2 tm = new TopicModeller2();
		tm.buildTopicModel("resources/corpus.txt");
		URL url;
		if (args.length > 1) {
			url = new File(args[1]).toURI().toURL();
		} else {
			url = TopicModeller2.class.getResource("config.xml");
		}
		...
		for (int x = 0; x < audioFiles.length; x++) {
			URL audioURL;
			if (args.length > 0) {
				audioURL = new File(args[0]).toURI().toURL();
			} else {
				audioURL = TopicModeller2.class.getResource(audioFiles[x]);
			}
			...
			WordSequenceAligner.Alignment a = werEval.align(INPUTS[x], hyp);
			...
			double maxValueInMap = (Collections.max(testprobabilities.values())); // This
																					// will
																					// return
																					// max
																					// value
																					...
																					// the
																					// Hashmap
			for (Entry<Integer, Double> entry : testprobabilities.entrySet()) { // Iterate
																				// through
																				// hashmap
				if (entry.getValue() == maxValueInMap) {
					highesttopic = entry.getKey();
					System.out.println("Highest probability topic: "
							+ highesttopic); // Print the key with max value
				}
			}

			// get words associated with highest probability topic

			String words = topicwords.get(highesttopic);
			//String words = null;
...
//			}
			...
			System.out.println(wordsrank);
			
			tm.adaptProbs(wordsrank);
			
			URL audioURLNew = TopicModeller2.class.getResource(audioFiles[x]);
			dataSource.setAudioFile(audioURLNew, null);
			done = false;
			while (!done) {
				...
				if (result != null) {
					System.out.println("Here i am in null!");
					Lattice lattice = new Lattice(result);
					LatticeOptimizer optimizer = new LatticeOptimizer(lattice);
					optimizer.optimize();
					lattice.dumpAllPaths();
					resultText = result.getBestResultNoFiller();
					System.out.println("Second time, I heard: " + resultText + '\n');
				} else {
					done = true;
					System.out.println("Here i am in true!");
				}
			}
